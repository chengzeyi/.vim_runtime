augroup myGui
    ...
    nnoremap <S-Up> :let &guifont = substitute(&guifont, '\(:h\<bar> \)\zs\d\+', '\=eval(submatch(0)+1)', 'g')<cr>
    nnoremap <S-Down> :let &guifont = substitute(&guifont, '\(:h\<bar> \)\zs\d\+', '\=eval(submatch(0) > 1 ? submatch(0)-1 : submatch(0))', 'g')<cr>
